...
Beside the wildcard listeners, port control binds address-specific sockets on the WAN IPv4 (all SO_REUSEPORT; the kernel delivers a connection to the most specific bound socket): the SNI demux on 443 while hostname routes hold it, and an HTTP→HTTPS redirect on 80 while WAN 443 is published (`http_redirect.rs`). The redirect also binds `0.0.0.0:8880`, the target of a wan-zone DNAT (UCI section `startwrt_http_redirect`) that rewrites WAN-side tcp/80 to it for as long as 443 is published. fw4 accepts a DNAT'd connection at input, so no ACCEPT rule opens port 80, and with the daemon down the DNAT lands on a closed port rather than on the UI's wildcard socket. Neither socket is wired into the axum app — the redirect can serve nothing but a 307.
...
| `port_control.rs`                 | Automatic port forwarding: PCP + UPnP IGD servers on the shared `start-core` protocol cores, mapping device-requested forwards onto auto-tagged UCI redirects with in-memory lease expiry; also owns the SNI demux and the WAN:80 redirect lifecycle |
| `http_redirect.rs`                | HTTP→HTTPS redirect while 443 is published (StartOS publishes only 443 and leaves this to its gateway, as StartTunnel does): `WAN:80` for LAN hairpins, `0.0.0.0:8880` behind a wan-zone tcp/80 DNAT for the Internet side; never the app router     |
