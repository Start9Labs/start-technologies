...
Port 80 is decided in the request path instead (`http_redirect.rs`). An outermost layer on the router answers a request whose destination is the WAN address with a 307 to the same authority over HTTPS, and passes everything else through, so the decision is live from the first accepted connection rather than from a socket bound later. The Internet side reaches it through a WAN ACCEPT rule on tcp/80 that port control keeps in the SNI admission set while 443 is published; sharing the `SNI` label is what makes a build that predates the redirect purge the rule rather than leave it admitting plain HTTP to a daemon with no layer in it. The gate the layer reads is the presence of that rule, seeded from UCI before the listener binds — a hostname route does not survive a restart but the rule it earned does, so the wanted state and the enforced state disagree exactly at startup. Until port control resolves the WAN address the layer redirects rather than falling through: on a double-NAT WAN, address class does not separate it from the LAN.
