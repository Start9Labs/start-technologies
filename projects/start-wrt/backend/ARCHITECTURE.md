...
| `http_redirect.rs`                | HTTP→HTTPS redirect bound to the WAN IPv4 on port 80 while 443 is published (StartOS publishes only 443 and leaves this to its gateway, as StartTunnel does); never the app router, never a firewall change                                          |
