...
Port 80 is decided in the request path instead (`http_redirect.rs`). An outermost layer on the router serves the UI to a client on a subnet the router is connected to off the WAN, at an address that is not the WAN's, and answers every other IPv4 request with a 307 to the same authority over HTTPS, so the decision is live from the first accepted connection rather than from a socket bound later. The Internet side reaches it through a WAN ACCEPT rule on tcp/80 that port control keeps in the SNI admission set while 443 is published; sharing the `SNI` label is what makes a build that predates the redirect purge the rule rather than leave it admitting plain HTTP to a daemon with no layer in it. The rule matches the wan zone rather than a destination, so a WAN-side host that routes the LAN subnet through the router arrives at the LAN address; the client's subnet is what keeps it from the UI. The rule's `name` tells the redirect's hold on port 80 from a hostname route's, which takes precedence. The gate opens before the rule is written and shuts only after the reload that removes it, and is seeded before the listener binds from any `SNI` rule on 80 — a hostname route does not survive a restart but the rule it earned does, so the wanted state and the enforced state disagree exactly at startup. The router's addresses are read at the same point and again on every sweep and WAN hotplug; unreadable, they leave no client trusted.
...
| `http_redirect.rs`                | HTTP→HTTPS redirect while 443 is published (StartOS publishes only 443 and leaves this to its gateway, as StartTunnel does): an outermost router layer keyed on the connection's destination, plus the WAN tcp/80 admission rule behind it                   |
