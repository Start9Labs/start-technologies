...
| ----------------------------------------------- | ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| **eMMC — hardware boot partitions** (boot0/boot1) | `bootinfo` header + FSBL (u-boot SPL), FSBL mirror in boot1 | eMMC boot entry point — the K1 BootROM tries microSD first, then boots eMMC from boot0. Written by StartWRT's boot0 provisioning (flash wizard and sysupgrade), not by the raw user-area copy |
