...
A published port is a public resource, so this works from every [Security Profile](security-profiles.md) that could reach it from the Internet — one whose **WAN Access** is **All** or **Blacklist**, outside any [blackout window](security-profiles.md#wan-blackout) — and from every profile whose **Access** setting already includes the target device's profile. A profile with neither — **WAN Access** set to **None** or **Whitelist**, and no Access to the device — is not given this route: for its connections to the router's public address, the router answers instead of the device. The device's global IPv6 address is its public address, and is reachable on the published port from the same profiles. Ports opened through [automatic port forwarding](#automatic-port-forwarding) are served the same way.
