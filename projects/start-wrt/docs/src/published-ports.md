...
It works from the target device's own [Security Profile](security-profiles.md) and from every other profile whose **Access** setting includes that profile — the same access those profiles already grant on the LAN, just by a different address. A profile that is not allowed to reach the device is not given this route either: for its connections to the router's public address, the router answers instead of the device. Ports opened through [automatic port forwarding](#automatic-port-forwarding) are served the same way.
