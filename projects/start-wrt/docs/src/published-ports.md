...
While port 443 is published — by a rule of yours, or by a device's [hostname routes](#hostname-routes-shared-ports) — a plain `http://` connection to the router's public address, or to a domain that points there, is answered with a redirect to `https://` rather than by the router's own web interface. That holds from your own network and from the Internet, whatever [Remote Access](settings.md#remote-access) is set to: the redirect is all that answers plain HTTP at the router's public address while 443 is published, so the router's web interface is never what a visitor reaches there, and with Remote Access off, port 80 answers the redirect instead of refusing the connection. A published domain therefore works when typed without `https://`. StartOS servers rely on this: they publish only 443 and leave the redirect to the router. A rule of yours that publishes port 80 takes precedence over the redirect, without a prompt, and so does a device's hostname route on port 80. When nothing is published on 443, port 80 at the router's public address answers as it always has, under the Remote Access setting.
