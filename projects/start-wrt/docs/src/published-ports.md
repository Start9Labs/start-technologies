...
Hostname routes carry connections arriving from the internet at your public address. For devices on your own network, have the name resolve directly to its device instead: give the device [DNS record publishing](devices.md#device-detail-page) permission, and on a StartOS server enable the domain as a **private domain** — registering a hostname route alone does not publish the name to your LAN, because exposing a service to the internet and disclosing it to your local network are separate decisions.
