...
While port 443 is published — by a rule of yours, or by a device's [hostname routes](#hostname-routes-shared-ports) — a plain `http://` connection to the router's public address, or to a domain that points there, is answered with a redirect to `https://` rather than by the router's own web interface. A published domain therefore works when typed without `https://`, from your own network and from wherever [Remote Access](settings.md#remote-access) already accepts port 80 from the WAN side. StartOS servers rely on this: they publish only 443 and leave the redirect to the router. A rule of yours that publishes port 80 takes precedence over the redirect. Nothing about who may reach port 80 from outside changes — that remains the Remote Access setting — only what answers there; and when nothing is published on 443, port 80 at the router's public address answers as it always has.
