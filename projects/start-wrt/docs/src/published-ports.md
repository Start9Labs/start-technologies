...
While port 443 is published — by a rule of yours, or by a device's [hostname routes](#hostname-routes-shared-ports) — plain `http://` to the router's public address, or to a domain that points there, is answered with a redirect to `https://` instead of the router's web interface. This holds from your own network and from the Internet, whatever [Remote Access](settings.md#remote-access) is set to, so a published domain works when typed without `https://`. StartOS servers rely on it: they publish only 443. A rule of yours or a hostname route that publishes port 80 takes precedence over the redirect.
