...
- **Allow DNS record publishing** — Off by default. Toggle on to let this device publish DNS names for itself into the router, so every device on your network can resolve them. A StartOS server uses this for its **private domains**: enable one on the server and it publishes the name here automatically. A device can only publish names that point at its own address, and a name is served only to networks whose Security Profile can reach that device. Enabling asks for confirmation; grant it only to a device you trust. Published names appear in a read-only table below the toggle. The device renews or withdraws its own records, a record lapses when the device loses the address it points at, and turning the toggle off removes them immediately.
