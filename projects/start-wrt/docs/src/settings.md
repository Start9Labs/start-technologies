...
> [!NOTE]
> While port 443 is published to a device — by a [Published Port](published-ports.md) or a device's [hostname routes](published-ports.md#hostname-routes-shared-ports) — a plain `http://` connection to the router's WAN address is answered with a redirect to `https://` instead of the web interface, so a published domain works when typed without `https://`. This holds under every option above, from your own network and from the Internet: the redirect is all that answers port 80 at the WAN address while 443 is published, even with Remote Access set to "Never", and the web interface is never reachable there over plain HTTP. When nothing is published on 443, port 80 follows the option above as before. See [Reaching a Published Port from Your Own Network](published-ports.md#reaching-a-published-port-from-your-own-network).
