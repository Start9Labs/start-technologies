...
- **`openwrt/` is a disposable, gitignored checkout of pristine upstream OpenWrt** (no submodule, no fork). `make start-wrt-openwrt-setup` fetches the release pinned in [`build/openwrt-version`](build/openwrt-version) and applies the Start9 delta: [`openwrt-patches/`](openwrt-patches/) (modified upstream files, `git apply`-ed) + [`openwrt-overlay/`](openwrt-overlay/) (added files, rsynced over the tree). Every setup run **resets the checkout** — never commit or keep work inside it; change the patch/overlay dirs instead (workflow in [CONTRIBUTING.md](CONTRIBUTING.md#openwrt-tree-pinned-upstream--patches--overlay)). The binary build does *not* need the checkout, only the full image does.
