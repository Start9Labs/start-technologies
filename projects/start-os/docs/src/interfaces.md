...
| Column                    | Description                                                                                                                                                                                                                                                                                                                                           |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Toggle**                | Enable or disable the address. Your server serves an address while it is enabled, and refuses HTTPS connections to it once you switch it off — see the note below for the two cases where it goes on answering. Public IPv4 addresses are off by default. All other addresses are on by default.                                                      |
...
| **Certificate Authority** | Who signs the SSL certificate for this address: **Root CA** (your server's own CA), **Let's Encrypt** (publicly trusted), **Self signed** (the service presents its own certificate), or `-` for an address served without SSL. The column appears only where the interface has an address served over HTTPS.                                         |
...
> - An interface your server does not serve over HTTPS goes on answering — plain HTTP, and the raw-TCP interfaces such as a P2P or ZMQ endpoint. Those have no **Certificate Authority** column, and their URLs carry no `https://`. A connection to one of them names no address, so switching one of its addresses off does not stop that interface answering there.
