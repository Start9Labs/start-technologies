...
| Column                    | Description                                                                                                                                                                                                                                                                                                                                           |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Toggle**                | Enable or disable the address. Your server serves an address while it is enabled and refuses connections to it once you switch it off. Public IPv4 addresses are off by default. All other addresses are on by default.                                                                                                                               |
...
> [!NOTE]
> Switching an IP address off leaves the names on that gateway alone — a [public domain](clearnet.md), a [private domain](private-domains.md) and your server's `.local` name are addresses in their own right, each with its own row. A gateway that has an enabled **public IPv4** address is a further exception: your router presents traffic from the Internet under the same local address that gateway already has, so your server cannot tell a local connection from a forwarded one and goes on serving every IPv4 address there. Switch the public IPv4 address off as well to take that gateway's IPv4 addresses down.
