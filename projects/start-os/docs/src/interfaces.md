...
| Column                    | Description                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Toggle**                | Enable or disable the address. Your server serves an address while it is enabled, and refuses SSL connections to it once you switch it off. See the note below for gateway-level exceptions. Public IPv4 addresses are off by default. All other addresses are on by default.                                                                                                                             |
...
> [!NOTE]
> Switching an IP address off stops it serving the interface over SSL. An SSL connection names the address it is for, so a [public domain](clearnet.md) or a [private domain](private-domains.md) on the same gateway is unaffected — each is an address in its own right, with its own row. Two gateway-level exceptions apply:
>
> - While a gateway has an enabled **public IPv4** address, every IPv4 address on that gateway accepts SSL connections. Your router presents traffic from the Internet under the same local address that gateway already has, so your server cannot tell a local connection from a forwarded one. Switch the public IPv4 address off to close SSL access through the gateway's other IPv4 addresses.
> - For an interface served without SSL, StartOS forwards its port per gateway rather than per IP. While any private address using that port is enabled, the port accepts connections at every IP on that gateway. It closes on the gateway after the last such address is disabled. This applies to plain HTTP and raw-TCP interfaces such as P2P or ZMQ; rows that share a non-SSL domain are switched together.
