...
> [!WARNING]
> This is one switch for the whole server, and it takes effect immediately. Every service you have installed that has a non-SSL address gains it on that gateway's directly connected network and on routed private networks at once, and those addresses are enabled as soon as they are offered — there is no per-service confirmation. A service you normally reach over HTTPS may still have a plaintext leg, and anything on those networks can read and alter traffic to it — including the passwords you type into it.
>
> Mark a gateway secure only when you control every device on its directly connected network and on any routed private network that can reach your server. Leave a guest network, an office LAN, a coffee-shop WiFi, or any network carrying devices you do not manage as it is.

An address unlocked by a secure gateway is available from the gateway's directly connected network and from routed private networks. StartOS does not open a port on your router for it or carry it on a [public domain](clearnet.md). On a server whose interface holds a public IP directly — a VPS, or a modem in bridge mode — that directly connected network is your provider's network, which contains machines you do not control.
