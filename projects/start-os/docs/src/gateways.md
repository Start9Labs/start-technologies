...
> [!WARNING]
> This is one switch for the whole server, and it takes effect immediately. Every service you have installed that has a non-SSL address gains it on that gateway at once, and those addresses are enabled as soon as they are offered — there is no per-service confirmation. A service you normally reach over HTTPS may still have a plaintext leg. Over IPv4, anything on the directly connected subnet or on a routed private network can read and alter traffic to it — including the passwords you type into it.
>
> Mark a gateway secure only when you control every device on its directly connected network and on every routed private IPv4 network that can reach your server. Leave a guest network, an office LAN, a coffee-shop WiFi, or any network carrying devices you do not manage as it is.

An IPv4 address unlocked by a secure gateway is available from the gateway's directly connected subnet and from any routed private IPv4 network that can reach the server. A raw IPv6 **Local** port is available from the directly connected subnet. Public/WAN access is controlled separately through [Public addresses and domains](clearnet.md). On a server whose interface holds a public IP directly — a VPS, or a modem in bridge mode — that directly connected network is your provider's network, which contains machines you do not control.
