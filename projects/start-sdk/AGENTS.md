...
- **`docs/skills/` is the packaging skills — guide content, not a book.** mdBook builds `src/` alone, but a skill change routes like a page: a fix to what is published goes to `live-docs` by PR and reaches every workspace and helix on their next sync; one that accompanies code rides that code's release. The repo-root `.claude/skills/package-service` is a symlink into it, for Claude Code in this repo and the `start9` plugin; the skill's own files stay here.
