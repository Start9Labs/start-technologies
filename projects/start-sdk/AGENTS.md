...
- **`docs/skills/` is the packaging skills — guide content, not a book.** mdBook builds `src/` alone, but a skill change routes like a page: a fix to what is published goes to `live-docs` by PR and reaches every workspace and helix on their next sync; one that accompanies code rides that code's release. Its readers point here — `init-workspace`'s workspace links, helix, the `start9` plugin manifest — so nothing at the repo root refers to it, and no product tag is involved in publishing a skill.
