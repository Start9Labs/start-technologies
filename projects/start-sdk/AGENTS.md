...
- **`docs/skills/` is the packaging skills — guide content, not a book.** mdBook builds `src/` alone, but a skill change routes like a page: a fix to what is published goes to `live-docs` by PR and reaches every workspace and helix on their next sync; one that accompanies code rides that code's release. Its readers point here — `init-workspace`'s workspace links, helix, and the repo-root `.claude-plugin/marketplace.json`, whose `start9` entry fetches this directory from `live-docs` by URL and declares the skills itself — so no product tag is involved in publishing a skill, and nothing plugin-specific lives in this directory. Keep a `package.json` out of it: a plugin install copies the directory and would install one.
