...
`addSsl.alpn` narrows the protocols the binding puts forward. A client that asks for protocols and shares none of them is refused; a client that asks for none at all is served as it would be without ALPN, and a list that names nothing turns nobody away.
