...
`addSsl.alpn` is a filter over the protocols the client asked for, leaving the ones this binding puts forward. A client that asks for protocols and shares none of them is refused; a client that asks for none at all is served as it would be without ALPN, and a list that names nothing turns nobody away.

What the list means beyond that depends on whether your container serves TLS of its own. Where it does — `secure: { ssl: true }` alongside `addSsl` — your container is offered those protocols the client also asked for, and the client is offered whatever your container picks, so the two ends stay on one protocol and a container that ignores ALPN entirely leaves the client without one. Where it does not, StartOS negotiates with the client itself and the list is simply what the client is offered, which is how `protocol: 'http'` puts `http/1.1` forward — and there the list is the only thing holding a client to what your container speaks, since nothing on the container's side gets a say. So pin it to your container's protocols on a binding StartOS terminates alone, and on a rewrap leave it unset unless you need to keep the binding off a protocol your container would otherwise select.
