...
A dynamic range is intersected with the base. A disjoint range throws; a broader range cannot loosen the published requirement. StartOS applies the same base when an enabled optional dependency is reported through `effects.setDependencies`, including its running status and health checks. `kind` may tighten from `exists` to `running`, and health checks may only be added. Returning `null` keeps the base. `.const(effects)` makes `enabled`, narrowing, and `withInit` reactive. Each dependency has its own effects context: a change reruns that dependency's handlers and publishes the combined requirements without rerunning siblings.

Both required and optional dependencies can use `.withInit` to create cross-service tasks. For optional dependencies, pass `.withInit(handler, ['dependency-id:action-id'])`, listing every task replay ID the handler creates. Those IDs are cleared when the integration is disabled, including after a container restart. `withInit` runs after the dependency is enabled and before the runtime requirements are published. A task created with `sdk.action.createTask` defaults to the replay ID `[package-id]:[action-id]`; specify a different ID in both the task creation and the list if needed.
