...
4. **Choose the daemon command from the image.** For images with an upstream startup command, use `sdk.useEntrypoint()` to keep that behavior. For images that contain only binaries (no Docker `CMD` or `ENTRYPOINT`), name the binary in `exec.command`; `s9pk pack` accepts them directly. If the image bundles an init/supervisor — `s6-overlay` (every `linuxserver/*` image), `tini`, `dumb-init`, `supervisord` — set `runAsInit: true` on the daemon's `exec`, or the supervisor crashes because it is not PID 1. See [Images with their own init system](#images-with-their-own-init-system).
...
- [ ] Daemon command matches the image (`sdk.useEntrypoint()` for an upstream startup command; explicit binary for a binaries-only image); `runAsInit: true` if the image has its own init system
