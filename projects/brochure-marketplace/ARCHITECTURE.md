...
3. `MarketplaceService` (extends `AbstractMarketplaceService` from the shared lib) owns registry selection state:
   ...
   - Start9's curated registries come from the manifest this site serves at `/.well-known/startos/registries.json`, which pins each one's name and icon. `@start9labs/marketplace`'s `resolveIdentity` renders those pinned values, so an unlisted registry cannot present a pinned registry's name; `identityMatches` gates adding a listed one whose live identity has drifted.
