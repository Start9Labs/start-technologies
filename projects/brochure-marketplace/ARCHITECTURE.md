...
3. `MarketplaceService` (extends `AbstractMarketplaceService` from the shared lib) owns registry selection state:
   ...
   - Start9's curated registries come from the manifest this site serves at `/.well-known/startos/registries.json`, which pins each one's name, icon, and description and carries the notice shown while it is selected. `@start9labs/marketplace` protects pinned names from impersonation, prefers valid live registry icons after fetch, and raises a persistent warning when a listed identity drifts.
