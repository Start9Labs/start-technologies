...
- **This site serves the verified-registries manifest that every StartOS server reads.** It lives in `@start9labs/shared` at `shared-libs/ts-modules/shared/well-known/startos/registries.json` and reaches this site through the `assets` entry in `angular.json`, never through `src/`. An entry's `name`, `icon`, `description`, `operator`, and `contact` are what the marketplace displays for that registry, whatever its server reports, and its `warning` shows while the registry is selected; `icon` is a data URL, since the OS UI renders it from a different origin, and `description` and `warning` are `LocaleString`s that carry every UI language. See the root [AGENTS.md](../../AGENTS.md#coupled-changes-keep-in-sync).
