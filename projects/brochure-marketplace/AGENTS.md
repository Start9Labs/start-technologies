...
- **This site serves the known-registries manifest that every StartOS server reads.** It lives at `well-known/startos/registries.json` (a build asset, not under `src/`) and pins the name, icon, and description of each registry Start9 lists, with the notice shown while that registry is selected. An entry's `name` must be what that registry actually serves from `registry.info`, and its `icon` and `description` must match whenever the registry serves one, or StartOS raises a persistent warning; `icon` is a data URL, since the OS UI renders it from a different origin. `warning` is a `LocaleString`, so carry every UI language. See the root [AGENTS.md](../../AGENTS.md#coupled-changes-keep-in-sync).
