...
pub async fn add_forward(
    ...
    let external_ip = ctx.external_ipv4(*target.ip()).await.ok_or_else(|| {
        Error::new(
            eyre!(
                "cannot publish a port for {}: this server holds no usable IPv4 address. Configure one on its network interface, or pin the address inbound traffic arrives on with `start-tunnel subnet <SUBNET> set-wan --wan-ip <IP>`",
                target.ip()
            ),
            ErrorKind::Network,
        )
