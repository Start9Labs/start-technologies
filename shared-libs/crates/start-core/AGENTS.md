...
- [Docker](https://docs.docker.com/get-docker/) for the builder and test containers
...
- `make start-core-test` — run the test suite (wraps `run-tests.sh`, which uses the `rust-zig-builder`
  ...
  `start9/cargo-zigbuild-test` from `build/test/testenv.Dockerfile`, adding the SquashFS tools
  required by s9pk integration tests to the standard builder. Or run a single test directly:
