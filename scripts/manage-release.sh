...
# The install guide's download links are reference definitions in a generated
# block running from this marker to EOF; the table referencing them is written
# by hand.
OS_INSTALL_DOC="projects/start-os/docs/src/installing-startos.md"
OS_DOWNLOADS_MARKER='<!-- Regenerated by `scripts/manage-release.sh sync-docs start-os`; `pre-check start-os` fails if these drift from the release being cut. -->'
...
# The install guide's generated link block for $VERSION, marker line included.
# Raspberry Pi links the .gz cmd_push_gz uploads beside the bare .img; that runs
# before `tag`, so the object exists by the time the tag publishes the book.
docs_download_block() {
    local platform ext url missing=""
    load_registry_index "$STARTOS_SOURCE_REGISTRY"
    printf '%s\n\n' "$OS_DOWNLOADS_MARKER"
    echo "[dl-release]: https://github.com/${REPO}/releases/tag/${TAG}"
    for platform in $OS_PLATFORMS; do
        for ext in $(os_image_exts "$platform"); do
            ...
            [ "$ext" != img ] || url="${url}.gz"
            echo "[dl-${ext}-${platform}]: ${url}"
        done
    done
    if [ -n "$missing" ]; then
        >&2 echo "${STARTOS_SOURCE_REGISTRY} indexes no ${VERSION} asset for:${missing}"
        return 1
    fi
}

# What the install guide currently carries, for diffing against the above.
docs_download_block_current() {
    local doc line
    doc="$REPO_ROOT/$OS_INSTALL_DOC"
    line=$(grep -nxF "$OS_DOWNLOADS_MARKER" "$doc" | head -1 | cut -d: -f1) || true
    if [ -z "$line" ]; then
        >&2 echo "${OS_INSTALL_DOC} has no generated download block"
        return 1
    fi
    tail -n +"$line" "$doc"
}
...
cmd_pre_check() {
    ...
    if [ "$KIND" = os ]; then
        ...
        if [ "$good" -eq 0 ]; then
            >&2 echo "  ✗ start-os docs link to no ${TAG} release — run './scripts/manage-release.sh sync-docs start-os'"
            errors=1
        ...
        # 1c. The install guide's download links point at this version's images
        # on the CDN. Their basenames carry the build's commit hash, so nothing
        # reconstructs them from a version number — regenerate and diff. Held to
        # a warning when the source registry has no build of this version yet:
        # check 4 below is where that gets reported.
        local want have
        if want=$(docs_download_block 2>/dev/null); then
            if have=$(docs_download_block_current 2>/dev/null) && [ "$have" = "$want" ]; then
                echo "  ✓ ${OS_INSTALL_DOC} downloads point at ${VERSION}"
            else
                >&2 echo "  ✗ ${OS_INSTALL_DOC} download links are stale — run './scripts/manage-release.sh sync-docs start-os'"
                diff <(printf '%s\n' "$have") <(printf '%s\n' "$want") | sed 's/^/      /' >&2 || true
                errors=1
            fi
        else
            >&2 echo "  ! download links not checked — no ${VERSION} images in ${STARTOS_SOURCE_REGISTRY}"
        fi
...
# Point the StartOS book at this version: the install guide's download block and
# every release link in it. Needs the images in the source registry, and has to
# land on master before `release` — the tag is what publishes the book.
cmd_sync_docs() {
    require_kind os
    local doc block line docs_src pinned
    doc="$REPO_ROOT/$OS_INSTALL_DOC"
    docs_src="$REPO_ROOT/projects/start-os/docs/src"
    block=$(docs_download_block)

    mapfile -t pinned < <(grep -rlE "${REPO}/releases/tag/start-os/v" "$docs_src" || true)
    if [ "${#pinned[@]}" -gt 0 ]; then
        sed -i -E "s|(${REPO}/releases/tag/)start-os/v[0-9A-Za-z.-]*|\1${TAG}|g" "${pinned[@]}"
    fi

    line=$(grep -nxF "$OS_DOWNLOADS_MARKER" "$doc" | head -1 | cut -d: -f1) || true
    if [ -n "$line" ]; then
        head -n "$((line - 1))" "$doc" > "${doc}.tmp"
    else
        cp "$doc" "${doc}.tmp"
        printf '\n' >> "${doc}.tmp"
    fi
    ...
    echo "Synced the StartOS docs to ${TAG}:"
    (cd "$REPO_ROOT" && git diff --stat -- "$docs_src" | sed 's/^/  /')
}
...
Subcommands:
  ...
  sync-docs          os only: rewrite the install guide's download links from
                     the source registry's index and re-pin every release link
                     in the book to this tag. The images must already be in the
                     source registry; commit the result to master before
                     `release`, since the tag is what publishes the book.
